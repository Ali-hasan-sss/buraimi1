...
export default async function CollegeCouncilDashboardPage() {
    const t = await getTranslations("dashboardCouncil");

    await dbConnect();

    const members = (await CollegeCouncilMember.find({}).sort({ createdAt: -1 }).lean()) as CollegeDoc[];

    return (
        <div className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-1">
                    <h1 className="text-2xl font-semibold tracking-tight">{t("collegeTitle")}</h1>
                    <p className="text-sm text-muted-foreground">{t("manageMembers")}</p>
                </div>
                <Button asChild className="w-full sm:w-auto">
                    <Link href="/dashboard/council/college-council/new">{t("addMember")}</Link>
                </Button>
            </div>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                                <Button asChild variant="outline" size="sm" className="w-full sm:w-auto">
                                                    <Link href={`/dashboard/council/college-council/${String(m._id)}`}>{t("update")}</Link>
                                                </Button>
